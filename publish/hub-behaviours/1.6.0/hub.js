/* hub-behaviours v1.6 — the controlled behaviour library.
 *
...
register("carousel", {
  setup(el) {
    ...
    // a pair of round controls over its left and right, "none" no controls at
    // all, leaving the rail to swipe, keyboard and scrollbar. A word this does
    ...
    let placement = (el.getAttribute("data-hub-carousel-controls") || "under")
      .trim().toLowerCase();
    if (placement !== "none" && placement !== "edges") placement = "under";
    // Edges lays the controls over the rail, which needs a containing block
    // that holds still. Where the block IS the scroller they would ride away
    // with the photographs, so the row goes back underneath. The attribute
    // below says which placement is on the page, so the answer is readable
    // rather than guessed at.
    if (placement === "edges" && scroller === el) placement = "under";
...
window.HubBehaviours = { version: "1.6.0", initAll, register };
