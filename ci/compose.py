#!/usr/bin/env python3
"""Generate shells/ - pre-assembled page shells - from the recipes.

Usage:
  python ci/compose.py            regenerate shells/
  python ci/compose.py --check    fail if shells/ differs from what this
                                  script would generate (CI runs this)

A shell is a recipe from ci/page-recipes.json promoted to a deliverable:
...
cannot drift apart. Everything under shells/ comes out of this script;
...
rewrites one file per shell rather than all of them.

Exit codes: 0 = clean. 1 = a composed page fails its page-level checks, or
--check found committed output stale. 2 = a shell cannot be built at
...
# scans shells/ - the scan happens here instead, at the moment the text
...
def compose_one(recipe, library):
    """One shell's files, as {filename: text}."""
    spec = recipe["composition"]
    name, version = spec["name"], spec["version"]
    where = f"shells/{name}@{version}"
...
def compose_readme(recipe, name, version, page, chosen, support):
    lines = [
        ...
        "(`masthead-nav` in this library); this shell ships without "
        "one.",
        "",
    ]
...
def generate():
    """Everything under shells/, as {relative path: text}."""
    _prove_drop_safety()
    ...
    if len(set(names)) != len(names):
        die(f"shell names collide: {sorted(names)}")
...
def only_the_release_tag_moved(rel, held, fresh):
    ...
    shell stamps it with the tag its branch was on, another merge
...
def main():
    ap = argparse.ArgumentParser(
        description="Generate shells/ from ci/page-recipes.json.")
    ...
    if args.check:
        ...
        for rel, text in sorted(files.items()):
            ...
            if held != text and not only_the_release_tag_moved(rel, held, text):
                findings.append(f"shells/{rel}: stale-shell: does "
                                f"not match the patterns and recipes; run ci/compose.py")
        if OUT.is_dir():
            for path in sorted(OUT.rglob("*")):
                rel = path.relative_to(OUT).as_posix()
                if path.is_file() and rel not in files:
                    findings.append(f"shells/{rel}: stray-file: nothing "
                                    f"generates this; run ci/compose.py")
        if findings:
            print("\n".join(findings))
            print(f"\n{len(findings)} finding(s).")
            return 1
        print(f"clean: {len(files)} generated file(s) match shells/.")
        return 0
