...
2. **hero-centred** v6 - **the opening region's default**, in `opening-default.html`, not in `page.html` - Opener with the claim centred above a wide cropped photograph in its own band - the one hero that never lays a word over an image, so no scrim has to carry a ratio.
...
5. **cta-image** v4 - **the closing region's default**, in `closing-default.html`, not in `page.html` - Full-bleed photographic closing call to action - one landscape image, a scrim carrying the contrast, and a centred claim with both platform controls over it.
