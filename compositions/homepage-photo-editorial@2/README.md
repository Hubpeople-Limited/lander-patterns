...
- **The opening** - `hero-centred` is the shipped default. A page may open instead on another opener from INDEX.md, on the content's own first row, or on nothing above the content at all. The recipe's `opens:` line, or the material, decides.
- **The closing** - `cta-image` is the shipped default. A page may close instead on a line in the prose, a single link, a quiet panel or the last row of its content. The recipe's `closes:` line decides; every page keeps one visible way to act.
