...
register("tabs", {
  setup(el) {
    if (el.previousElementSibling &&
        el.previousElementSibling.classList.contains("hub-tabs-list")) return;
    const panels = Array.from(el.children).filter((child) =>
      child.hasAttribute("data-hub-tab-label"));
    ...
    // Both listeners live on the rail this module made, so they go when it
    // goes; nothing is attached to the document.
...
/* !important: a pattern styles the panel's display, and [hidden] must win. */
